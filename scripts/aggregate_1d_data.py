#!./.venv/bin/python3
"""
Script to aggregate 1d frequency zipped CSV files into a single parquet file.
This script:
1. Identifies 1d frequency files in the raw data directory
...
SAVE_PATH = Path(__file__).parent.parent / "data" / "1d" / "1d.parquet"
...
def extract_symbol_and_date(filename: str) -> tuple[str, str, str]:
    ...
    Example: '1INCHUSDT-1d-2022-08.zip' -> ('1INCHUSDT', '2022', '08')
    """
    pattern = r"^(.+?)-1d-(\d{4})-(\d{2})\.zip$"
...
def csv_has_header(csv_data: bytes) -> bool:
    ...
    This is more reliable than date-based detection since some files (e.g., BNXUSDT-1d-2022-06)
...
def main():
    print("Starting 1d data aggregation...")
    print(f"Raw data path: {RAW_PATH}")
    print(f"Save path: {SAVE_PATH}")

    # Find all 1d frequency zip files
    pattern = "*-1d-*.zip"
    zip_files = sorted(RAW_PATH.glob(pattern))

    if not zip_files:
        print(f"No 1d frequency files found in {RAW_PATH}")
        return

    print(f"Found {len(zip_files)} 1d frequency files")
