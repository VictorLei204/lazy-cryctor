...
curl -s -H 'Content-Type: application/json'  https://api.binance.com/api/v3/exchangeInfo | jq -r '.symbols | sort_by(.symbol) | .[] | select(.symbol | test("USDT$")) | .symbol' > symbols.txt

curl -s -H 'Content-Type: application/json'  https://fapi.binance.com/fapi/v1/exchangeInfo | jq -r '.symbols | sort_by(.symbol) | .[] | select(.symbol | test("USDT$")) | .symbol' > um_symbols.txt
